  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/ask_bot', {
        ...
        body: JSON.stringify({
          conversationsNew: messages.concat(userMessage),
          question: input,
        }),
      });
          ...
          <form onSubmit={handleSubmit} className="p-4 border-t border-gray-700">
            <div className="flex items-center space-x-2 bg-[#2A2A2A] rounded-full p-2">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="bg-[#3A3A3A] text-white p-2 rounded-full hover:bg-[#4A4A4A] transition-colors disabled:opacity-50"
              >
                <Send size={20} />
              </button>
            </div>
  ...
  <div className="bg-[#2A2A2A] p-4 rounded-lg shadow-md border border-gray-700">
    ...
    <ul className="space-y-2">
      {suggestions.map((suggestion, index) => (
        <li key={index} className="text-sm text-white cursor-pointer hover:text-gray-300 transition-colors">
          {suggestion}
        </li>
      ))}
    </ul>
